...
class AssessmentApplicationTests {
	...
	void contextLoads() throws Exception
	{
		...
		logger.info("Makin insert list of cars");
		carService.deleteAll(); 
		//Dados site https://www.noticiasautomotivas.com.br/top-10-carros-populares-mais-economicos/
		carService.save(new Car("Ford Ka 1.0"," Ka 1.0","Ford", "2021", 13.14, 15.15));
		carService.save(new Car("Fiat Mobi Drive","1.0 GSR","Fiat" ,"2021", 14.00 , 15.90));
		carService.save(  new Car("Fiat Argo Drive 1.0","E63 AMG","Fiat", "2021", 14.20, 15.10));
		carService.save(new Car("Peugeot 208 1.2","208 1.2","Peugeot", "2021",13.90,15.50));
		carService.save(new Car("Renault Kwid","1.0","Renault", "2021",14.90,15.60));
		carService.save(new Car( "Chevrolet Onix","1.0 Onix","Chevrolet", "2021", 13.90, 16.70));
		carService.save( new Car("Fiat Uno 1.0","Uno 1.0","Fiat", "2021", 13.20, 15.20));
		carService.save(new Car("Chevrolet Onix Turbo","1.0 Turbo","Chevrolet", "2021", 8.10, 12.6));
		carService.save( new Car("Chevrolet Joy 1.0","Joy 1.0","Chevrolet", "2021",12.80, 15.20));
        carService.save(new Car("Volkswagen up!","1.0 TSI","Volkswagen", "2021", 14.30, 16.30));
	
		logger.info("Makin mock object CarRequest");
	...
	{
		logger.info("Test Ranking list name cars");
    ...
    public void testRankinCarCsHwaysService() throws Exception
	 {
		logger.info("Test Ranking list car and consumption in hways");
		List<RankingCarResponse>colRankin= carService.rankingList(10.00, 10.00, 5.00); 
		for(int a=0 ;a< colRankin.size();a++  ){
			RankingCarResponse rankin= colRankin.get(a);
			switch (a) {
    ...
    public void testRankinCarCsCityService() throws Exception
	 {
		logger.info("Test Ranking list car and consumption in city");
		List<RankingCarResponse>colRankin= carService.rankingList(10.00, 10.00, 5.00); 
		for(int a=0 ;a< colRankin.size();a++  ){
			RankingCarResponse rankin= colRankin.get(a);
			switch (a) {
	...
	@Test
	public void rankinCarSaveController() throws Exception 
	{
		logger.info("API:Test service REST POST save Controller Ranking");
		String jsonRequest = gson.toJson(
									new CarRequest(
										"Ford Fiesta 1.4","Fiesta 1.4","Ford", "2021", 10.14, 13.15
									)
								);
		mockMvc.perform(
	...
	@Test
	public void rankinCarListController() throws Exception 
	{
		logger.info("API:Test service REST GET LIST Car Was Ranking Controller Ranking");
		
		mockMvc.perform(
							get("/raking/car/list?totalKMCity=10.10&totalKMHways=10.10&gasolPrice=4.50")
